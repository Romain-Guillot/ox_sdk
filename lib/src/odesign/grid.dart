...
enum OColumnSortOrder { asc, desc }
...
class OGridColumnSpacer<T> extends OGridColumn<T> {
  OGridColumnSpacer([double? width]) : super(key: GlobalKey(), width: width ?? 15.0, label: '', renderer: (_, __) => Container());
}

class OGridHeaderStyle {
  const OGridHeaderStyle({this.decoration, this.textStyle, this.verticalSpacing});
...
class ODataGridTheme {
  const ODataGridTheme({this.headerStyle, this.rowStyle});
...
class _Header<T> extends StatefulWidget {
  const _Header({Key? key, required this.theme, required this.columns, required this.density}) : super(key: key);
...
class __HeaderState<T> extends State<_Header<T>> {
  ...
  Widget build(BuildContext context) {
    Widget header = DefaultTextStyle.merge(
        style: theme?.headerStyle?.textStyle ?? Theme.of(context).textTheme.titleMedium,
        child: Builder(
            builder: (BuildContext context) => Row(
                  children: columns.map(
                    (OGridColumn<T> column) {
                          ...
                          child: Wrap(
                            ...
                            children: [
                              ...
                              Icon(
                                  ...
                                  color: sortColumn == column ? Theme.of(context).colorScheme.onSurface : Theme.of(context).disabledColor)
                            ],
                          ),
                        );
                      }
                      ...
                      if (isFixed) {
                        ...
                        return Expanded(flex: column.weight, child: child);
                      }
                    },
                  ).toList(),
                )));
    ...
    header = Container(
      padding: EdgeInsets.symmetric(
          vertical: theme?.headerStyle?.verticalSpacing ?? 0.0, horizontal: (theme?.rowStyle?.padding(widget.density).horizontal ?? 0) / 2),
      child: header,
    );
    ...
    if (theme?.headerStyle?.decoration != null) {
      header = Container(
        ...
        child: Material(color: Theme.of(context).colorScheme.surface, child: header),
      );
    }
        ...
        SliverList(
          ...
          (context, index) {
            ...
            Widget child = Container(
              ...
              constraints: BoxConstraints(minHeight: effectiveTheme?.rowStyle?.minHeight?.call(density) ?? 0.0),
              ...
              child: DefaultTextStyle.merge(
                ...
                child: Row(
                  children: columns.map((OGridColumn<T> column) {
                    final isFixed = column.width != null;
                    final child = Align(alignment: column.alignment, child: column.renderer(index, item));
                    if (isFixed) {
                      return SizedBox(width: column.width, child: child);
                    } else {
                      return Expanded(flex: column.weight, child: child);
