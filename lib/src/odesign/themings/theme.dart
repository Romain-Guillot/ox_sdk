...
enum LayoutDensity { narrow, wide }
...
ThemeData theme(LayoutDensity density, {ColorScheme colorScheme = colorScheme}) {
  TextTheme textThemeFontSizes;
  switch (density) {
    case LayoutDensity.narrow:
      textThemeFontSizes = const TextTheme(
          displaySmall: TextStyle(fontSize: 25),
    ...
    case LayoutDensity.wide:
      textThemeFontSizes = const TextTheme(
          displaySmall: TextStyle(fontSize: 27),
  ...
  final radiuses = RadiusesTheme(
      tiny: BorderRadius.circular(4), small: BorderRadius.circular(10), medium: BorderRadius.circular(15), big: BorderRadius.circular(22));
  ...
  return ThemeData(
      ...
      dialogTheme: DialogTheme(
        shape: RoundedRectangleBorder(borderRadius: radiuses.medium),
      ...
      navigationBarTheme: NavigationBarThemeData(
        ...
        iconTheme: MaterialStateProperty.resolveWith((states) {
          return IconThemeData(color: states.contains(MaterialState.selected) ? const Color(0xFFF2F2F2) : Colors.black);
        }),
      ),
      ...
      chipTheme: ChipThemeData(
          ...
          labelStyle: textThemeFontSizes.bodyMedium?.copyWith(color: Colors.grey[700]),
          elevation: 2,
      ...
      inputDecorationTheme: const InputDecorationTheme(
          ...
          labelStyle: TextStyle(fontSize: 14, color: Color.fromARGB(255, 114, 114, 114), fontWeight: FontWeight.bold, letterSpacing: 1.15)),
      textButtonTheme: TextButtonThemeData(
          style: TextButton.styleFrom(
              foregroundColor: Colors.white, backgroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: radiuses.small))),
      popupMenuTheme: PopupMenuThemeData(elevation: 8, shape: RoundedRectangleBorder(borderRadius: radiuses.medium)),
      useMaterial3: true,
      textTheme: textThemeFontSizes
          .copyWith(
            displaySmall: textThemeFontSizes.displaySmall?.copyWith(fontWeight: FontWeight.bold),
            displayMedium: textThemeFontSizes.displayMedium?.copyWith(fontWeight: FontWeight.bold),
            titleSmall: textThemeFontSizes.titleSmall?.copyWith(fontWeight: FontWeight.bold),
            titleMedium: textThemeFontSizes.titleMedium?.copyWith(fontWeight: FontWeight.w900),
            bodyMedium: textThemeFontSizes.bodyMedium?.copyWith(fontWeight: FontWeight.bold),
            bodyLarge: textThemeFontSizes.bodyLarge?.copyWith(fontWeight: FontWeight.bold),
            headlineSmall: textThemeFontSizes.headlineSmall?.copyWith(fontWeight: FontWeight.w900),
            labelLarge: textThemeFontSizes.labelLarge?.copyWith(fontWeight: FontWeight.bold),
            titleLarge: textThemeFontSizes.titleLarge?.copyWith(fontWeight: FontWeight.bold),
          )
          .apply(bodyColor: Colors.black, displayColor: Colors.black),
      colorScheme: colorScheme,
      extensions: [
        ButtonThemes(
          error: TextButton.styleFrom(foregroundColor: colorScheme.onErrorContainer, backgroundColor: colorScheme.errorContainer),
          success: TextButton.styleFrom(foregroundColor: colors.onSuccess, backgroundColor: colors.success),
        ),
        colors,
        radiuses,
        const PaddingsTheme(medium: 20, big: 30, small: 12, tiny: 3),
        const ConstraintsTheme(maxPageWidth: 1024, mobileScreenMax: kBreakpointLarge, snackbarMaxSize: 800),
        ComponentsTheme(
          tooltip: const OTooltipThemeData(backgroundColor: Colors.black, style: TextStyle(color: Colors.white, fontWeight: FontWeight.normal)),
              ...
              rowStyle: GridRowStyle(
                  padding: (density) {
                    if (density == VisualDensity.compact) {
                      return const EdgeInsets.symmetric(vertical: 0, horizontal: 18);
                    } else if (density == VisualDensity.comfortable) {
                      return const EdgeInsets.symmetric(vertical: 8, horizontal: 18);
                    } else {
